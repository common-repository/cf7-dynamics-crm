...
*/
  ...
  vxcf_dynamics::$data=vxcf_dynamics::get_data_object();
global $pagenow; 
...
if(isset($res['error'])){
  ...
  */
  ...
  vxcf_dynamics::$data->delete_feed($feed_id);
  }
  ?>
  <div class="updated fade" style="margin:10px 0;">
  <p>
  ...
  $feeds = vxcf_dynamics::$data->get_feeds(); 
...
$count=vxcf_dynamics::$data->delete_log($log_ids);
  ...
  $log = vxcf_dynamics::$data->get_log_by_id($id); 
  ...
  $data= vxcf_dynamics::$data->get_log(); $items=count($data['feeds']);
...
include_once(vxcf_dynamics::$path . "templates/fields-mapping.php"); 
  ...
  */
  ...
  $feed = vxcf_dynamics::$data->get_feed($id);
  vxcf_dynamics::$data->update_feed(array("is_active"=>vxcf_dynamics::post("is_active")),$id);
  } 
  ...
  */
  ...
  vxcf_dynamics::$data->update_feed_order(vxcf_dynamics::post('sort'));
  }
...
$feeds=vxcf_dynamics::$data->get_object_feeds($form_id,$account,$object,$skip);
...
$log=vxcf_dynamics::$data->get_log_by_id($log_id); 
...
$msgs=array();
$feed= vxcf_dynamics::$data->get_feed($fid); 
...
if(is_array($feed_update) && is_array($feed)){
  ...
  $is_valid=vxcf_dynamics::$data->update_feed($feed_update,$fid);
